...
public static class Derivatives
{
    ...
    /// <summary>
    /// Downloads only the FragmentList.pack files and extracts a lightweight dbID-to-location mapping.
    /// This is significantly more memory-efficient than <see cref="ReadFragmentsRemoteAsync"/> because
    /// it only retains the translation position and bounding box per unique dbID, discarding material,
    /// geometry, and other fragment data. Ideal for combining with property database queries.
    /// </summary>
    /// <param name="urn">The URL-safe Base64 encoded URN of the source design.</param>
    /// <param name="accessToken">The access token for authentication.</param>
    /// <returns>
    /// A dictionary mapping each unique dbID to its <see cref="FragmentLocation"/>
    /// (translation position + bounding box).
    /// </returns>
    public static async Task<Dictionary<int, FragmentLocation>> ReadFragmentLocationsRemoteAsync(
        string? urn, string accessToken)
    {
        List<Resource> resourcesToDownload = await ExtractSvfAsync(urn, accessToken).ConfigureAwait(false);
        resourcesToDownload = resourcesToDownload.FindAll(r => r.FileName == "FragmentList.pack");

        var allLocations = new Dictionary<int, FragmentLocation>();
        ...
        foreach (Resource resource in resourcesToDownload)
        {
            ...
            var data = await ExecuteRequestData(request, resource).ConfigureAwait(false);
            if (data != null)
            {
                var locations = FragmentLocationParser.Parse(data);
                foreach (var kvp in locations)
                {
                    allLocations.TryAdd(kvp.Key, kvp.Value);
                }
            }
        }
    ...
    /// <summary>
    /// Downloads only the FragmentList.pack files and extracts locations only for the specified dbIDs.
    /// This is useful when you already know which elements you need (e.g., from an SDB property query)
    /// and want to avoid building a location index for the entire model.
    /// </summary>
    /// <param name="urn">The URL-safe Base64 encoded URN of the source design.</param>
    /// <param name="accessToken">The access token for authentication.</param>
    /// <param name="targetDbIds">The set of dbIDs to retrieve locations for.</param>
    /// <returns>
    /// A dictionary mapping each requested dbID (that was found) to its <see cref="FragmentLocation"/>.
    /// </returns>
    public static async Task<Dictionary<int, FragmentLocation>> ReadFragmentLocationsFilteredRemoteAsync(
        string? urn, string accessToken, HashSet<int> targetDbIds)
    {
        List<Resource> resourcesToDownload = await ExtractSvfAsync(urn, accessToken).ConfigureAwait(false);
        resourcesToDownload = resourcesToDownload.FindAll(r => r.FileName == "FragmentList.pack");
        ...
        foreach (Resource resource in resourcesToDownload)
        {
            ...
            if (data != null)
            {
                var locations = FragmentLocationParser.ParseFiltered(data, targetDbIds);
                foreach (var kvp in locations)
                {
                    allLocations.TryAdd(kvp.Key, kvp.Value);
                }
    ...
    public static async Task<List<Resource>> ExtractSvfAsync(string? urn, string accessToken)
    {
        ...
        Manifest manifest = await modelDerivativeClient.GetManifestAsync(urn, accessToken: accessToken);
        // get the manifest for the URN

        // list items of the manifest file
        List<ManifestItem> urns = ParseManifest(manifest.Derivatives[0]?.Children);
    ...
    private static List<ManifestItem> ParseManifest(List<ManifestResources> manifest)
    {
        List<ManifestItem> urns = new List<ManifestItem>();
        foreach (ManifestResources item in manifest)
        {
            if (!string.IsNullOrEmpty(item.Role) && Roles.Contains(item.Role))
            {
                urns.Add(new ManifestItem
                {
                    Guid = item.Guid,
                    MIME = item.Mime,
                    Path = DecomposeUrn(item.Urn)
                });
            }

            if (item.Children != null && item.Children.Count > 0)
            {
                urns.AddRange(ParseManifest(item.Children));
            }
        }
